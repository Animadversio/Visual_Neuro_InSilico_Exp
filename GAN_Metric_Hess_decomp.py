...
if __name__ == "__main__":
    # %% Prepare the Networks: Perceptual Similarity Net Gan alexnet or VGG
    import sys
    ...
    objective = FeatLinModel(alexnet, layername='features_10', type="neuron", chan=10, pos=(10, 10)) # slice(None)
    ...
    reg = LinearRegression().fit(eig_log[eig_log > -3].reshape(-1, 1), vHv_log[eig_log > -3].reshape(-1, 1))
    ...
    #%%
    sort_idx = residue.argsort()
    sort_idx_flt = sort_idx[(eig_log[sort_idx] > -3) & (vHv_log[sort_idx] > -3.5)]
    eig_id_arr = sort_idx_flt[-15:]
    tuning_plot(G, preprocess, objective, eigvals, eigvects, eig_id_arr, veclabel="SU_top_inv", ticks=21, titlestr="top log(eig) - log(metric vHv) vectors for both > 1E-3")
    ...
    plt.savefig(join(savedir, "Activ-vHv-Ratio_single_chan_lin_%02d.jpg"%(RND)))
    plt.show()
    #%
    plt.scatter(np.log10(np.abs(eigvals)), np.log10(np.abs(vHv_arr)))
    plt.xlabel("Eigenvalue of Activation function (log)")
    plt.ylabel("vHv of metric function (log)")
    plt.savefig(join(savedir, "Activ-vHv-Ratio_single_chan_log_%02d.jpg"%(RND)))
    plt.show()
    #%%
    ...
    print(time() - t0)
    #%% %time
    met_eigval, met_eigvec = np.linalg.eigh(H)
