...
def sim_hessian_computation(z, percept_loss, savepath=None):
    ...
    """
    feat = torch.from_numpy(z).requires_grad_(False)
    img_ref = G.visualize(feat,)  # forward the feature vector through the GAN
    # Perturbation vector for gradient
    perturb_vec = torch.zeros((1, 4096), dtype=float32).requires_grad_(True)
    perturb_vec = 0.00001*torch.randn((1, 4096), dtype=float32).requires_grad_(True)
