...
#%%
ckpt_path = join(StyleGAN_root, "checkpoint", "model.ckpt-533504.pt")
size = 512
device = "cpu"
latent = 512
n_mlp = 8
...
def visualize_batch_np(codes_all_arr, truncation, mean_latent, B = 5):
    csr = 0
    img_all = None
    ...
    while csr < imgn:
        ...
        with torch.no_grad():
            img_list = G.visualize(torch.from_numpy(codes_all_arr[csr:csr_end, :]).float().cuda(), truncation=truncation, mean_latent=mean_latent).cpu()
...
#%%
        ...
        img_all = visualize_batch_np(codes_all_arr, truncation=truncation, mean_latent=mean_latent, B=5)
