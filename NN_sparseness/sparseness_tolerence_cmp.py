"""
Recreate main results of Zocollan 2007 in silico.
"""
...
for li, layer_long in enumerate(feattsrs.keys()):
    ...
    #%%
    maxresp_per_obj = tuning_tsr.max(dim=1, keepdim=True).values
    # formula fixed on July 6th, 2022
    Tfmtoler = (((tuning_tsr / maxresp_per_obj).sum(dim=1) - 1) / (Ntfm - 1)).nanmean(dim=[0])
    sparseness = 1 - (INrespmat.mean(dim=0) ** 2) / (INrespmat ** 2).mean(dim=0)
    df = pd.DataFrame({"layeridx": li, "layer": layer_short, "layerlong": layer_long,
                          "sparseness": sparseness, "tolerance": Tfmtoler})
    df_all = df_all.append(df, ignore_index=True) if df_all is not None else df
    #%%
    valmsk = (~sparseness.isnan()) & (~Tfmtoler.isnan())
    rho, pval = pearsonr(sparseness[valmsk], Tfmtoler[valmsk])
#%%
g = sns.FacetGrid(df_all, col="layer", height=4, aspect=1.0,
          sharex=True, xlim=(0.0, 1), ylim=(0, 1))
g.map(sns.scatterplot, "tolerance", "sparseness", alpha=0.25)
g.map(annotate_corrfunc, "tolerance", "sparseness", xy=(.05, .1))
saveallforms(figdir, f"{netname}_tolerance-sparseness_per_layer_pd", g.fig)
plt.show()
