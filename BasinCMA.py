...
def BasinCMA(target_tsr, cmasteps=30, gradsteps=40, finalgrad=500, batch_size=4,
             basis="all", CMApostAdam=False, RND=None):
    ...
    CMAfitimg = ToPILImage()(make_grid(fit_img[:, :, :, :].cpu()))
    ...
    fig.savefig(join(savedir, "traj_H%s%s_%d_dsim_%.3f_L1_%.3f.jpg" % (basis, "_postAdam" if CMApostAdam else "", RND, scores_final.min(), L1score_final.min())))
    np.savez(join(savedir, "optim_data_%d.npz" % RND), codes=latent_code.cpu().detach().numpy(), Record=Record, dsims=scores_final, L1dsims=L1score_final)
...
for imgid in tqdm.trange(200, 205):
    ...
    imgs_np, codes_np, dsims, L1dsims, Record = BasinCMA(target_tsr, cmasteps=10, gradsteps=10, finalgrad=500, batch_size=4, basis="all", CMApostAdam=True)

    imgs_np, codes_np, dsims, L1dsims, Record = BasinCMA(target_tsr, cmasteps=10, gradsteps=10, finalgrad=500, batch_size=4, basis="all", CMApostAdam=False)
...
# plt.savefig(join(savedir, "traj_Hallreg%06d_%.3f.jpg" % (RND, dsim.item())))
# np.savez(join(savedir, "code_Hallreg%06d.jpg" % (RND,)), dsim=dsim.item(), scores_all=np.array(scores_all),
